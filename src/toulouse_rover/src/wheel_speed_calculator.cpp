...
BaseWheelSpeedCalculator::BaseWheelSpeedCalculator(
...
WheelSpeedCalculator::WheelSpeedCalculator(
    ros::NodeHandle& nh, const std::string wheel_namespace,
    util::WheelConfigurationType wheel_config_type)
    : wheel_namespace_(wheel_namespace), wheel_config_type_(wheel_config_type) {
  setupPubsSubs(nh, wheel_namespace);
  setupWheels(wheel_config_type_);
}

void WheelSpeedCalculator::velocityCallback(
...
void WheelSpeedCalculator::setupPubsSubs(ros::NodeHandle& nh,
                                         const std::string wheel_namespace) {
  wheel_speeds_pub_ = nh.advertise<toulouse_rover::WheelSpeeds>(
      "/" + wheel_namespace + "/wheel_cmd_speeds", 1);
  ctrl_sub_ = = nh.subscribe("cmd_vel", 1, velocityCallback);
}

void WheelSpeedCalculator::setupWheels(
...
void WheelSpeedCalculator::calcAndPublishWheelSpeeds(
    const geometry_msgs::Twist& velocity_msg) {
  calcAllWheelSpeeds(velocity_msg);
  publishWheelSpeeds();
}

void WheelSpeedCalculator::calcAllWheelSpeeds(
    const geometry_msgs::Twist& velocity_msg) {
  wheel_speeds_msg_.header.stamp = ros::Time::now();
  if (wheel_config_type == util::WheelConfigurationType::OMNI_WHEELS ||
      wheel_config_type == util::WheelConfigurationType::SKID_STEERING) {
    wheel_speeds_msg_.front_left_radians_per_sec =
        front_left_wheel_.calcWheelSpeed(velocity_msg);
    wheel_speeds_msg_.front_right_radians_per_sec =
        front_right_wheel_.calcWheelSpeed(velocity_msg);
  }

  wheel_speeds_msg_.back_left_radians_per_sec =
      back_left_wheel_.calcWheelSpeed(velocity_msg);
  wheel_speeds_msg_.back_right_radians_per_sec =
      back_right_wheel_.calcWheelSpeed(velocity_msg);
}
...
float FrontLeftWheelSpeedCalculator::calcWheelSpeed(
    const geometry_msgs::Twist& cmd_vel_msg) {
...
float FrontRightWheelSpeedCalculator::calcWheelSpeed(
    const geometry_msgs::Twist& cmd_vel_msg) {
...
float BackRightWheelSpeedCalculator::calcWheelSpeed(
    const geometry_msgs::Twist& cmd_vel_msg) {
...
float BackLeftWheelSpeedCalculator::calcWheelSpeed(
    const geometry_msgs::Twist& cmd_vel_msg) {
