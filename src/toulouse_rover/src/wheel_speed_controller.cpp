...
int BaseWheelSpeedController::pwmFromWheelSpeed(float wheel_speed)
{
  ...
  {
    ROS_WARN("Wheel commanded to %f (absolute) radians per sec, flooring to the min of %f", wheel_speed,
             util::MIN_WHEEL_SPEED);
    wheel_speed = util::MIN_WHEEL_SPEED;
  }
  else if (wheel_speed > util::MAX_WHEEL_SPEED)
  {
    ROS_WARN("Wheel commanded to %f (absolute) radians per sec, capping to the max of %f", wheel_speed,
             util::MAX_WHEEL_SPEED);
