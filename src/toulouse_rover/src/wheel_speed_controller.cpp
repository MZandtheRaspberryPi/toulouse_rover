...
BaseWheelSpeedController::BaseWheelSpeedController(
    const std::string wheel_namespace, bool use_pid,
...
FrontLeftWheelSpeedController::FrontLeftWheelSpeedController(
    std::string wheel_namespace, bool use_pid,
    util::WheelConfigurationType wheel_config_type)
    : BaseWheelSpeedController(nh, wheel_namespace, use_pid, odom_type) {
  encoderIndex_ = 0;
}
...
FrontRightWheelSpeedController::FrontRightWheelSpeedController(
    std::string wheel_namespace, bool use_pid,
    util::WheelConfigurationType wheel_config_type)
    : BaseWheelSpeedController(nh, wheel_namespace, use_pid, odom_type) {
  encoderIndex_ = 1;
...
BackRightWheelSpeedController::BackRightWheelSpeedController(
    std::string wheel_namespace, bool use_pid,
    util::WheelConfigurationType wheel_config_type)
    : BaseWheelSpeedController(nh, wheel_namespace, use_pid, odom_type) {
  encoderIndex_ = 2;
...
BackLeftWheelSpeedController::BackLeftWheelSpeedController(
    std::string wheel_namespace, bool use_pid,
    util::WheelConfigurationType wheel_config_type)
    : BaseWheelSpeedController(nh, wheel_namespace, use_pid, odom_type) {
  encoderIndex_ = 3;
...
BaseWheelSpeedController::setupPubsSubs(ros::NodeHandle& nh,
                                        const std::string wheel_namespace) {
  state_pub_ =
  ...
  if (use_pid_) {
    ctrl_sub_ =
        nh.subscribe("/" + wheel_namespace + "/control_effort", 1,
                     &BaseWheelSpeedController::controlEffortCallback, this);
  }
}

void BaseWheelSpeedController::controlEffortCallback(
...
void BaseWheelSpeedController::publishSetpoint(
    const float& speed_radians_per_sec) {
...
float BaseWheelSpeedController::getWheelspeed() { return current_wheel_speed_; }
...
WheelSpeedController::WheelSpeedController(
    ros::NodeHandle& nh, std::string wheel_namespace, bool use_pid,
    util::WheelConfigurationType wheel_config_type)
    : wheel_namespace_(wheel_namespace),
      use_pid_(use_pid),
      wheel_config_type_(wheel_config_type) {
  ...
  setup_wheels(wheel_namespace_, use_pid, wheel_config_type);
  enableMotors();
  loop_rate_(loop_rate);
}

wheelSpeedController::setupCustomSignalHandlers() {
...
void WheelSpeedController::setupGPIO() {
  ...
  wiringPiISR(util::FRONT_LEFT_INTERUPT_PIN, INT_EDGE_FALLING,
              &wheels::frontLeftInterupt);
  wiringPiISR(util::FRONT_RIGHT_INTERUPT_PIN, INT_EDGE_FALLING,
              &wheels::frontRightInterupt);
  wiringPiISR(util::BACK_LEFT_INTERUPT_PIN, INT_EDGE_FALLING,
              &wheels::backLeftInterupt);
  wiringPiISR(util::BACK_RIGHT_INTERUPT_PIN, INT_EDGE_FALLING,
              &wheels::backRightInterupt);
#endif
}

void WheelSpeedController::setupPubsSubs(ros::NodeHandle& nh,
                                         const std::string wheel_namespace) {
  wheel_speed_sub_ = nh.subscribe("/" + wheel_namespace_ + "/wheel_cmd_speeds",
                                  1, wheelSpeedCallback);
  // servos_absolute publisher
  ros::Publisher servos_absolute_pub_ =
      nh.advertise<i2cpwm_board::ServoArray>("servos_absolute", 1);

  encoder_pub_ = nh.advertise<toulouse_rover::WheelAdjEncoderCounts>(
      "wheel_adj_enc_count", 1);

  wheel_speed_actual_pub_ =
      nh.advertise<toulouse_rover::WheelSpeeds>("wheel_speeds", 1);
}

void WheelSpeedController::wheelSpeedCallback(
    const toulouse_rover::WheelSpeed::ConstPtr& wheel_speeds) {
  const std::lock_guard<std::mutex> lock(speedUpdateMutex);
  wheel_speeds_ = *wheel_speeds;

  globalSpeedCounter[front_left_speed_ctrl_.encoderIndex_] =
      wheel_speeds_.front_left_radians_per_sec;
  globalSpeedCounter[front_right_speed_ctrl_.encoderIndex_] =
      wheel_speeds_.front_right_radians_per_sec;
  globalSpeedCounter[back_right_speed_ctrl_.encoderIndex_] =
      wheel_speeds_.back_right_radians_per_sec;
  globalSpeedCounter[back_left_speed_ctrl_.encoderIndex_] =
      wheel_speeds_.back_left_radians_per_sec;
}
...
void WheelSpeedController::setup_wheels(
    const std::string& wheel_namespace, const bool& use_pid,
    const util::WheelConfigurationType& wheel_config_type) {
  if (wheel_config_type == util::WheelConfigurationType::OMNI_WHEELS ||
      wheel_config_type == util::WheelConfigurationType::SKID_STEERING) {
    front_left_speed_ctrl_(wheel_namespace_ + "/front_left_wheel", use_pid,
                           wheel_config_type);
    front_right_speed_ctrl_(wheel_namespace_ + "/front_right_wheel", use_pid,
                            wheel_config_type);
  }
  back_right_speed_ctrl_(wheel_namespace_ + "/back_right_wheel", use_pid,
                         wheel_config_type);
  back_left_speed_ctrl_(wheel_namespace_ + "/back_left_wheel", use_pid,
                        wheel_config_type);
}

WheelSpeedController::publish_wheel_setpoints(
    const toulouse_rover::WheelSpeeds& wheel_spds_to_ctrl) {
  if (wheel_config_type == util::WheelConfigurationType::OMNI_WHEELS ||
      wheel_config_type == util::WheelConfigurationType::SKID_STEERING) {
    front_left_speed_ctrl_.publishSetpoint(
        wheel_spds_to_ctrl.front_left_radians_per_sec);
    front_right_speed_ctrl_.publishSetpoint(
        wheel_spds_to_ctrl.front_right_radians_per_sec);
  }
  back_right_speed_ctrl_.publishSetpoint(
      wheel_spds_to_ctrl.back_right_radians_per_sec);
  back_left_speed_ctrl_.publishSetpoint(
      wheel_spds_to_ctrl.back_left_radians_per_sec);
}

WheelSpeedController::publish_wheel_states() {
  ...
  if (wheel_config_type == util::WheelConfigurationType::OMNI_WHEELS ||
    ...
    current_speeds.front_left_radians_per_sec =
        front_left_speed_ctrl_.getWheelspeed();

    front_right_speed_ctrl_.publishWheelState();
    current_speeds.front_right_radians_per_sec =
        front_right_speed_ctrl_.getWheelspeed();
  }
  back_right_speed_ctrl_.publishWheelState();
  current_speeds.back_left_radians_per_sec =
      back_right_speed_ctrl_.getWheelSpeed();
  back_left_speed_ctrl_.publishWheelState();
  current_speeds.back_right_radians_per_sec =
      back_left_speed_ctrl_.getWheelSpeed();
...
controlEffort WheelSpeedController::get_control_efforts() {
  ...
  float front_left_control_effort, front_right_control_effort,
      back_left_control_effort, back_right_control_effort = 0.0;
  if (wheel_config_type == util::WheelConfigurationType::OMNI_WHEELS ||
      wheel_config_type == util::WheelConfigurationType::SKID_STEERING) {
    control_efforts.front_left_control_effort =
        front_left_speed_ctrl_.spinAndWaitForCtrlEffort();
    control_efforts.front_right_control_effort =
        front_right_speed_ctrl_.spinAndWaitForCtrlEffort();
  }

  control_efforts.back_right_control_effort =
      back_right_speed_ctrl_.spinAndWaitForCtrlEffort();
  control_efforts.back_left_control_effort =
      back_left_speed_ctrl_.spinAndWaitForCtrlEffort();
  return control_efforts;
}

void WheelSpeedController::updateAndPublishServoArray(
    const controlEffort& control_effort) {
  if (control_effort.front_left_control_effort < 0) {
    servo_array_.servos[14].value = 0;
    servo_array_.servos[15].value =
        std::abs(control_effort.front_left_control_effort);
  } else {
    servo_array_.servos[15].value = 0;
    servo_array_.servos[14].value =
        std::abs(control_effort.front_left_control_effort);
  }

  if (control_effort.front_right_control_effort < 0) {
    servo_array_.servos[13].value = 0;
    servo_array_.servos[12].value =
        std::abs(control_effort.front_right_control_effort);
  } else {
    servo_array_.servos[12].value = 0;
    servo_array_.servos[13].value =
        std::abs(control_effort.front_right_control_effort);
  }

  if (control_effort.back_left_control_effort < 0) {
    servo_array_.servos[9].value = 0;
    servo_array_.servos[8].value =
        std::abs(control_effort.back_left_control_effort);
  } else {
    servo_array_.servos[8].value = 0;
    servo_array_.servos[9].value =
        std::abs(control_effort.back_left_control_effort);
  }

  if (control_effort.back_right_control_effort < 0) {
    servo_array_.servos[10].value = 0;
    servo_array_.servos[11].value =
        std::abs(control_effort.back_right_control_effort);
  } else {
    servo_array_.servos[11].value = 0;
    servo_array_.servos[10].value =
        std::abs(control_effort.back_right_control_effort);
  }
...
void publishAdjEncoderData() {
  ...
  wheel_adj_encoder_counts_.front_left_adjusted_encoder_count =
      globalEncCounter[front_left_speed_ctrl_.encoderIndex_];
  wheel_adj_encoder_counts_.front_right_adjusted_encoder_count =
      globalEncCounter[front_right_speed_ctrl_.encoderIndex_];
  wheel_adj_encoder_counts_.back_right_adjusted_encoder_count =
      globalEncCounter[back_right_speed_ctrl_.encoderIndex_];
  wheel_adj_encoder_counts_.back_left_adjusted_encoder_count =
      globalEncCounter[back_left_speed_ctrl_.encoderIndex_];
  encoder_pub_.publish(wheel_adj_encoder_counts_);
}

WheelSpeedController::zeroOutMotors() {
  for (int i = 0; i < 16; i++) {
    servo_array_[i] = 0;
  }
  servos_absolute_pub_.publish(servo_array_);
...
void WheelSpeedController::spin() {
  while (!wheel_speed_controller::g_request_shutdown) {
    const std::lock_guard<std::mutex> lock(speedUpdateMutex);
    publish_wheel_setpoints(wheel_speeds_);
