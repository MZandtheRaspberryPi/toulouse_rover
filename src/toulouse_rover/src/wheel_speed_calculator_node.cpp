...
int main(int argc, char** argv) {
  ...
  if (!nh.getParam("speed_controller_loop_rate", loop_rate)) {
    ROS_WARN(
        "wheel_speed_calculator_node: param speed_controller_loop_rate is not "
        "set.");
  }

  wheel_speed_calculator::WheelSpeedCalculator wheel_calc(
      nh, wheel_namespace_str, wheel_config_type, loop_rate);
  wheel_calc.spin();
}